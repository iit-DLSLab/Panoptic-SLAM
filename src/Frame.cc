...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
     ...
     mTcw(frame.mTcw), mbHasPose(false), mbHasVelocity(false), Panoptic_objs(frame.Panoptic_objs), stuff_keypoints(frame.stuff_keypoints), stuff_descriptor(frame.stuff_descriptor),
     unknown_keypoints(frame.unknown_keypoints), unknown_descriptor(frame.unknown_descriptor), all_masks(frame.all_masks)
{ //thing_keypoints(frame.thing_keypoints), thing_descriptor(frame.thing_descriptor)
...
void Frame::SetPanopticResults(cv::Mat &people_mask){

    peopleMask = people_mask.clone();
}

void Frame::RemoveOutliers(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera)
{
    ...
    cv::Mat currentMask = peopleMask.clone();
    cv::Mat Mask_gray;
    cv::cvtColor(currentMask,Mask_gray,cv::COLOR_BGR2GRAY);

    for (size_t i(0); i < mvKeys.size(); ++i)
    {
        
        int val = static_cast<int>(Mask_gray.at<uchar>(mvKeys[i].pt.y,mvKeys[i].pt.x));
...
void Frame::Set_Panoptic_Image(cv::Mat panoptic) { panoptic.copyTo(panoptic_image);}

void Frame::set_stuff_keypoints(std::vector<cv::KeyPoint> keypoints) {stuff_keypoints = keypoints;}

void Frame::set_thing_keypoints(std::map<int,std::vector<cv::KeyPoint>> &keypoints) 
{
    thing_keypoints.clear();
    thing_keypoints.insert(keypoints.begin(), keypoints.end());

}

void Frame::set_unknown_keypoints(std::vector<cv::KeyPoint> keypoints) {unknown_keypoints = keypoints;}

void Frame::setPanoptic(std::vector<Panoptic_Object> objs) {Panoptic_objs.assign(objs.begin(),objs.end());}

void Frame::set_stuff_descriptor(cv::Mat descriptor) {stuff_descriptor = descriptor.clone();}

void Frame::set_thing_descriptor(std::map<int,cv::Mat> &descriptor) 
{
    thing_descriptor.clear();
    thing_descriptor.insert(descriptor.begin(),descriptor.end());

}

void Frame::set_unknown_descriptor(cv::Mat descriptor) {unknown_descriptor = descriptor.clone();}

void Frame::set_panoptic_masks(cv::Mat masks) {all_masks = masks.clone();}

void Frame::Remove_Moving_KeyPoints(std::vector<cv::KeyPoint> kps, const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera)
{
    if(mvKeys.empty())
        return;
