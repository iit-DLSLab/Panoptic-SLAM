...
namespace ORB_SLAM3
{
    ...
    int ORBmatcher::FeatureMatching(std::vector<cv::KeyPoint> curr_kp, cv::Mat curr_desc,std::vector<cv::KeyPoint> last_kp,cv::Mat last_desc, vector<int> &vnMatches12)
    {
        int nmatches=0;
        vnMatches12 = vector<int>(curr_kp.size(),-1);
        ...
        vector<int> vMatchedDistance(last_kp.size(),INT_MAX);
        vector<int> vnMatches21(last_kp.size(),-1);

        for(size_t i1=0, iend1=curr_kp.size(); i1<iend1; i1++)
        {
            cv::KeyPoint kp1 = curr_kp[i1];
            int level1 = kp1.octave;
            if(level1>0) {continue;}

            cv::Mat d1 = curr_desc.row(i1);
            ...
            for(size_t i2=0, iend2=last_kp.size(); i2<iend2; i2++)
            {

                cv::Mat d2 = last_desc.row(i2);
            ...
            if(bestDist<=TH_LOW)
            {
                if(bestDist<(float)bestDist2*mfNNratio)
                {
                    ...
                    if(mbCheckOrientation)
                    {
                        float rot = curr_kp[i1].angle-last_kp[bestIdx2].angle;
                        if(rot<0.0)
                            rot+=360.0f;
                        int bin = round(rot*factor);
                        if(bin==HISTO_LENGTH)
