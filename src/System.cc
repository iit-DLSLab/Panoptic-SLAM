    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    cout << endl <<
    "Panoptic-SLAM Copyright (C) 2024 Gabriel Fischer and João Soares, Istituto Italiano di Tecnologia." << endl <<
    ...
    //Initialize the Panoptic thread
    mpPanopticNet = new PanopticNet();
    mptPanopticNet = new thread(&ORB_SLAM3::PanopticNet::Run,mpPanopticNet);
    mpPanopticNet->SetTracker(mpTracker);

    //Set pointers between threads
    mpTracker->SetLocalMapper(mpLocalMapper);
    mpTracker->SetLoopClosing(mpLoopCloser);
    mpTracker->SetPanopticNet(mpPanopticNet);   //
...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    mpTracker->GetImgForPanoptic(im);
