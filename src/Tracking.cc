...
void Tracking::SetPanopticNet(PanopticNet *pPanopticNet)
{
    mpPanopticNet=pPanopticNet;
}
...
//------------------------------------------------------------
void Tracking::GetImgForPanoptic(const cv::Mat& img)
{
    unique_lock<mutex> lock(mpPanopticNet->mMutexGetNewImg);
    mpPanopticNet->mbNewImgFlag=true;
    img.copyTo(mpPanopticNet->mImg);
}

bool Tracking::isNewDetectedImgArrived()
{
    std::unique_lock <std::mutex> lock(mpPanopticNet->mMutexNewImgDetection);
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    //
    //std::chrono::steady_clock::time_point tdet1 = std::chrono::steady_clock::now();
    while(!isNewDetectedImgArrived()) 
    {
        usleep(1);
    }
    //std::chrono::steady_clock::time_point tdet2 = std::chrono::steady_clock::now();
    //double t_det= std::chrono::duration_cast<std::chrono::duration<double> >(tdet2 - tdet1).count();
    //cout << "time waiting for detection: " << t_det*1000 << endl;
    ...
    people_mask = cv::Mat::zeros(cv::Size(640, 480), CV_8UC3); 
    ...
    for(int i=0; i<panoptic_result.objs.size(); i++){
        ...
        if(panoptic_result.objs[i].category_id == 0){  
    ...
    mCurrentFrame.Remove_Moving_KeyPoints(Kp_toBe_Filtered,mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
...
void Tracking::Thing_Stuff_Detachment()
{
    std::vector<Panoptic_Object> current_panoptic_objs = mCurrentFrame.Panoptic_objs;
    
    std::vector<cv::KeyPoint> filtered_stuff_key_points;
    cv::Mat filtered_stuff_descriptor;

    //Filter Keypoints in the Current Frame
    for (unsigned int j=0; j< current_panoptic_objs.size(); j++) // for every panoptic mask
    {
        Panoptic_Object mask_obj = current_panoptic_objs[j];
        if(mask_obj.mask.empty()) {continue;}
        if(mask_obj.isThing && mask_obj.category_id==0) {continue;} // ignore people

        cv::Mat mask = mask_obj.mask.clone(); // get panoptic stuff mask
        cv::threshold( mask, mask, 1, 1, cv::THRESH_BINARY ); // masks have only 1 and 0 values
        cv::cvtColor(mask,mask,cv::COLOR_BGR2GRAY);
        int obj_id = mask_obj.category_id;                    // get object id label
        int tracking_id = mask_obj.tracking_id;
          
        // create tmp variables to store keypoins and descriptor from an object
        std::vector<cv::KeyPoint> tmp_thing_key_points;
        cv::Mat tmp_thing_descriptor;
        
        for (unsigned int i=0; i < mCurrentFrame.mvKeys.size(); i++) // check all Keypoints
        {
            cv::KeyPoint keypoint = mCurrentFrame.mvKeys[i];  // get a Keypoint
            cv::Mat descriptor = mCurrentFrame.mDescriptors.row(i); // get its descriptor
            int val = static_cast<int>(mask.at<uchar>(keypoint.pt.y,keypoint.pt.x));
            if(val==1) // if keypoint is inside mask, Keep Keypoint
            { 
                if(mask_obj.isThing)
                {
                    tmp_thing_key_points.push_back(keypoint);
                    tmp_thing_descriptor.push_back(descriptor);
                }
                else
                {
                    filtered_stuff_key_points.push_back(keypoint);
                    filtered_stuff_descriptor.push_back(descriptor);
                }
            }   
        }
        ...
        if(mask_obj.isThing)
        {
            ...
            if(tmp_thing_key_points.size() > 0)
            {
                curr_thing_keypoints.insert(std::make_pair(tracking_id,tmp_thing_key_points));
                curr_thing_descriptor.insert(std::make_pair(tracking_id,tmp_thing_descriptor));
            }
        }    
    ...
    //Update Frame object with filtered keypoints and descriptors
    mCurrentFrame.set_stuff_keypoints(filtered_stuff_key_points);
    mCurrentFrame.set_stuff_descriptor(filtered_stuff_descriptor);

}

//Filter Keypoints in void masks (void masks are set of pixels that are neither thing or stuff)
void Tracking::Unknown_Mask_Keypoints()
{
    cv::Mat all_masks = mCurrentFrame.all_masks.clone();
    cv::threshold( all_masks, all_masks, 1, 1, cv::THRESH_BINARY ); // masks have only 1 and 0 values
    cv::cvtColor(all_masks,all_masks,cv::COLOR_BGR2GRAY);
    ...
    for (unsigned int i=0; i < mCurrentFrame.mvKeys.size(); i++) // check all Keypoints
    {
        cv::KeyPoint keypoint = mCurrentFrame.mvKeys[i];  // get a Keypoint
        cv::Mat descriptor = mCurrentFrame.mDescriptors.row(i); // get its descriptor
        int val = static_cast<int>(all_masks.at<uchar>(keypoint.pt.y,keypoint.pt.x));
        if(val==0)
        {   
            unknown_keypoints.push_back(keypoint);
            unknown_descriptor.push_back(descriptor);
        }
    }
    mCurrentFrame.set_unknown_keypoints(unknown_keypoints);
    mCurrentFrame.set_unknown_descriptor(unknown_descriptor);

}

// Static Feature Detection
void Tracking::Find_Stuff_Fundamental_Matrix(cv::Mat &F,cv::Mat &mask_F)
{
    // Find correspondences (Feature Matching)
    std::vector<int> FeatureMatching;
    ORBmatcher matcher(0.9,true);
    int nmatches = matcher.FeatureMatching(mCurrentFrame.stuff_keypoints,mCurrentFrame.stuff_descriptor,
                                           mLastFrame.stuff_keypoints,mLastFrame.stuff_descriptor,FeatureMatching);

    if(nmatches > 40)
    {
        //Get matched keypoints
        std::vector<cv::Point2f> current_matched_stuff_keypoints;
        std::vector<cv::Point2f> last_matched_stuff_keypoints;
        Get_Matched_Keypoints(mCurrentFrame.stuff_keypoints,mLastFrame.stuff_keypoints,current_matched_stuff_keypoints,last_matched_stuff_keypoints,FeatureMatching);

        // F-Matrix
        mask_F = cv::Mat(cv::Size(1, 300), CV_8UC1);
        F = cv::findFundamentalMat(last_matched_stuff_keypoints,current_matched_stuff_keypoints,mask_F, cv::FM_RANSAC, 0.1, 0.99);
       
    }
   
}

// Dynamic Feature Detection for possible moving objects
void Tracking::Find_Thing_Fundamental_Matrix(cv::Mat Static_F, cv::Mat mask_F)
{                                           
    std::map<uint,cv::Mat> Fundamental_Matrix;  // map tracking id with fundamental matrix;
    std::vector<Panoptic_Object> current_panoptic_objs = mCurrentFrame.Panoptic_objs;
    std::vector<Panoptic_Object> last_panoptic_objs = mLastFrame.Panoptic_objs;

    for (unsigned int j=0; j< current_panoptic_objs.size(); j++) // for every panoptic mask
    {
        Panoptic_Object pan_obj = current_panoptic_objs[j];
        if(!pan_obj.isThing) {continue;}

        //get object tracking id
        int tracking_id = pan_obj.tracking_id;
        int category_id = pan_obj.category_id;

        // get position from current frame std::map using tracking id as Key
        std::map<int,std::vector<cv::KeyPoint>>::const_iterator kp_pos_curr = curr_thing_keypoints.find(tracking_id);
        std::map<int,cv::Mat>::const_iterator desc_pos_curr = curr_thing_descriptor.find(tracking_id);


        std::vector<cv::KeyPoint>  curr_keypoints = kp_pos_curr->second;
        cv::Mat curr_descriptor = desc_pos_curr->second.clone();
        if(curr_descriptor.empty() || curr_keypoints.empty()) {continue;}
      

        if(!pan_obj.isNewObj) // if it is not a new object
        {
            
            // get position from last frame std::map using tracking id as Key
            std::map<int,std::vector<cv::KeyPoint>>::const_iterator kp_pos_last = last_thing_keypoints.find(tracking_id);
            std::map<int,cv::Mat>::const_iterator desc_pos_last = last_thing_descriptor.find(tracking_id);

            // get keypoints and descritor values from respective tracking id
            std::vector<cv::KeyPoint> last_keypoints = kp_pos_last->second;
            cv::Mat last_descriptor = desc_pos_last->second.clone();
            
            // Find correspondences (Feature Matching)
            std::vector<int> FeatureMatching;
            ORBmatcher matcher(0.9,true);
            int nmatches = matcher.FeatureMatching(curr_keypoints,curr_descriptor,
                                                   last_keypoints,last_descriptor,FeatureMatching);


            if(nmatches > 0)
            {
                //Get matched keypoints
                std::vector<cv::Point2f> current_matched_thing_keypoints;
                std::vector<cv::Point2f> last_matched_thing_keypoints;
                Get_Matched_Keypoints(curr_keypoints,last_keypoints,current_matched_thing_keypoints,last_matched_thing_keypoints,FeatureMatching);

                int counter = 0;
                for (unsigned int i = 0; i < last_matched_thing_keypoints.size(); i++)
                {
                    
                    double A = Static_F.at<double>(0, 0)*last_matched_thing_keypoints[i].x + Static_F.at<double>(0, 1)*last_matched_thing_keypoints[i].y + Static_F.at<double>(0, 2);
                    double B = Static_F.at<double>(1, 0)*last_matched_thing_keypoints[i].x + Static_F.at<double>(1, 1)*last_matched_thing_keypoints[i].y + Static_F.at<double>(1, 2);
                    double C = Static_F.at<double>(2, 0)*last_matched_thing_keypoints[i].x + Static_F.at<double>(2, 1)*last_matched_thing_keypoints[i].y + Static_F.at<double>(2, 2);
                    double dd = fabs(A*current_matched_thing_keypoints[i].x + B*current_matched_thing_keypoints[i].y + C) / sqrt(A*A + B*B); //Epipolar constraints
                    if (dd > 0.1)
                    {
                        cv::KeyPoint kp;
                        counter++;
                        kp.pt.x = current_matched_thing_keypoints[i].x;
                        kp.pt.y = current_matched_thing_keypoints[i].y;
                        Kp_toBe_Filtered.push_back(kp);
                        
                    }
                    
                }

                float sz = current_matched_thing_keypoints.size() / 1.0;
                float perc = static_cast<float>(counter / sz);
                
            }
           
        }
        else  // for new object we filter the keypoint a priori
        {
            Kp_toBe_Filtered.insert(Kp_toBe_Filtered.end(),curr_keypoints.begin(), curr_keypoints.end());
        }
...
void Tracking::Find_Unknown_Fundamental_Matrix(cv::Mat Static_F)
{
    // Find correspondences (Feature Matching)
    std::vector<int> FeatureMatching;
    ORBmatcher matcher(0.9,true);
    int nmatches = matcher.FeatureMatching(mCurrentFrame.unknown_keypoints,mCurrentFrame.unknown_descriptor,
                                           mLastFrame.unknown_keypoints,mLastFrame.unknown_descriptor,FeatureMatching);

    if(nmatches > 0)
    {
        //Get matched keypoints
        std::vector<cv::Point2f> current_matched_unknown_keypoints;
        std::vector<cv::Point2f> last_matched_unknown_keypoints;
        Get_Matched_Keypoints(mCurrentFrame.unknown_keypoints,mLastFrame.unknown_keypoints,
                              current_matched_unknown_keypoints,last_matched_unknown_keypoints,FeatureMatching);

        std::vector<cv::KeyPoint> current_unmatched_unknown_keypoints;
        Get_Unmatched_Keypoints(mCurrentFrame.unknown_keypoints, current_unmatched_unknown_keypoints,FeatureMatching);
        
        //calculate matched KeyPoints with Static Fundamental Matrix
        int counter = 0;
        for (unsigned int i = 0; i < last_matched_unknown_keypoints.size(); i++)
        {
            
            double A = Static_F.at<double>(0, 0)*last_matched_unknown_keypoints[i].x + Static_F.at<double>(0, 1)*last_matched_unknown_keypoints[i].y + Static_F.at<double>(0, 2);
            double B = Static_F.at<double>(1, 0)*last_matched_unknown_keypoints[i].x + Static_F.at<double>(1, 1)*last_matched_unknown_keypoints[i].y + Static_F.at<double>(1, 2);
            double C = Static_F.at<double>(2, 0)*last_matched_unknown_keypoints[i].x + Static_F.at<double>(2, 1)*last_matched_unknown_keypoints[i].y + Static_F.at<double>(2, 2);
            double dd = fabs(A*current_matched_unknown_keypoints[i].x + B*current_matched_unknown_keypoints[i].y + C) / sqrt(A*A + B*B); //Epipolar constraints

            if (dd > 0.1)
            {
                counter++;
                cv::KeyPoint kp;
                kp.pt.x = current_matched_unknown_keypoints[i].x;
                kp.pt.y = current_matched_unknown_keypoints[i].y;
                Kp_toBe_Filtered.push_back(kp);
        ...
        //Unmatched unknown keypoints will be filtered
        if(!current_unmatched_unknown_keypoints.empty())
        {
            Kp_toBe_Filtered.insert(Kp_toBe_Filtered.end(),current_unmatched_unknown_keypoints.begin(),current_unmatched_unknown_keypoints.end());
        }  

    }
}

void Tracking::normalizePoints(std::vector<cv::Point2f>& points)
{
    ...
    float avg_dist = 0;
    for (const auto& p : points) {
        avg_dist += cv::norm(p - centroid);
    }
    avg_dist /= points.size();

    // create the transformation matrix
    cv::Matx33f T(1, 0, -centroid.x,
                  0, 1, -centroid.y,
                  0, 0, sqrt(2) / avg_dist);
    cv::Matx33f S(1, 0, 0,
                  0, 1, 0,
    ...
    for (auto& p : points) {
        cv::Matx31f p_homog(p.x, p.y, 1);
        p_homog = T * p_homog;
        p.x = p_homog(0) / p_homog(2);
        p.y = p_homog(1) / p_homog(2);
    }
}

void Tracking::Find_Whole_Fundamental_Matrix(cv::Mat &F, cv::Mat &mask_F)
{

    // Find correspondences (Feature Matching)
    std::vector<int> FeatureMatching;
    ORBmatcher matcher(0.9,true);
    int nmatches = matcher.FeatureMatching(mCurrentFrame.mvKeys,mCurrentFrame.mDescriptors,
                                           mLastFrame.mvKeys,mLastFrame.mDescriptors,FeatureMatching);

    if(nmatches > 16)
    {
        std::vector<cv::Point2f> curr_keypoints;
        std::vector<cv::Point2f> last_keypoints;
        Get_Matched_Keypoints(mCurrentFrame.mvKeys,mLastFrame.mvKeys,curr_keypoints,last_keypoints,FeatureMatching);

        mask_F = cv::Mat(cv::Size(1, 300), CV_8UC1);
        F = cv::findFundamentalMat(last_keypoints, curr_keypoints,mask_F,cv::FM_RANSAC, 0.1, 0.99);
...
void Tracking::Get_Unmatched_Keypoints(std::vector<cv::KeyPoint> &curr_keypoints,std::vector<cv::KeyPoint> &curr_unmatched_keypoints,std::vector<int> FeatureMatching)
{
    for (unsigned int i=0; i< FeatureMatching.size(); i++)
    {
        int elem = FeatureMatching[i];
        if(elem==-1) 
        {
            curr_unmatched_keypoints.push_back(curr_keypoints[i]);
        }
...
void Tracking::Get_Matched_Keypoints(std::vector<cv::KeyPoint> &curr_keypoints,std::vector<cv::KeyPoint> &last_keypoints,std::vector<cv::Point2f> &curr_matched_keypoints,std::vector<cv::Point2f> &last_matched_keypoints,std::vector<int> FeatureMatching)
{
    for (unsigned int i=0; i< FeatureMatching.size(); i++)
    {
        int elem = FeatureMatching[i];
        ...
        {
            curr_matched_keypoints.push_back(curr_keypoints[i].pt);
            last_matched_keypoints.push_back(last_keypoints[elem].pt);
        }
...
double Tracking::FrobeniusNorm(cv::Mat F)
{
    int sumSq = 0;
    int row = F.rows;
    int col = F.cols;
    for (int i = 0; i < row; i++) {
        for (int j = 0; j < col; j++) {
            sumSq += pow(static_cast<double>(F.at<uchar>(i,j)), 2);
        }
    }
