...
void Viewer::Run()
{
    ...
    pangolin::CreateWindowAndBind("Panoptic-SLAM: Map Viewer",1024,768);
    ...
    while(1)
    {
        ...
        cv::Mat Pim = mpFrameDrawer->pIm;
        cv::Mat Unkim = mpFrameDrawer->unkIm;
        ...
        if(!Pim.empty())
        {
            cv::imshow("Panoptic Segmentation",Pim);
            cv::waitKey(mT);
        }

        if(!Unkim.empty())
        {
            cv::imshow("Unknown",Unkim);
            cv::waitKey(mT);
        }
